...
class Event(Base):
    ...
    time_start: Mapped['time'] = mapped_column(Time, index=True, nullable=False, default=datetime.now(tz=pytz.timezone('Europe/Moscow')))

    date_end: Mapped['date'] = mapped_column(Date, index=True, nullable=True)
    time_end: Mapped['time'] = mapped_column(Time, index=True, nullable=True)
